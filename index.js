...
// ---------------------------------------------------------------------------
// CUSTOMER LOOKUP — NOT YET CONFIRMED against live Auckland data, unlike
...
async function fetchCustomers(type) {
  ...
  let page = 1;
  while (true) {
    const res = await simproRequest(`companies/${process.env.SIMPRO_COMPANY_ID}/customers/${type}/`, {
      pageSize: '100',
      page: String(page),
